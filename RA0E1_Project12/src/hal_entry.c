...
void hal_entry(void)
{
    ...
    err = R_UARTA_Write(&g_uart0_ctrl, send_buff, len);
    if(FSP_SUCCESS != err) __BKPT();
        while(uart_send_complete_flag == false){}
    uart_send_complete_flag = false;
...
//            err = R_UARTA_Write(&g_uart0_ctrl, g_dest, TRANSFER_LENGTH);
//            if(FSP_SUCCESS != err) __BKPT();
//                while(uart_send_complete_flag == false){}
//        }
        if(g_out_of_band_index==10)
        {
            ...
            err = R_UARTA_Write(&g_uart0_ctrl, g_out_of_band_received, TRANSFER_LENGTH_1);
            if(FSP_SUCCESS != err) __BKPT();
                while(uart_send_complete_flag == false){}
        }
